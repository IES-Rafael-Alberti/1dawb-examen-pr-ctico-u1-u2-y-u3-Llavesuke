...
def genera_pista(posicion_tesoro:tuple,posicion:tuple) -> tuple:
    """
...
def simbolo_celda(celda:str) -> str:
    ...
    :param celda: Cada valor de una fila"""
...
def imprimir_mapa_oculto(mapa: list,posicion_jugador:tuple):
    ...
    mapa_oculto = []
    print("    " +" ".join(str(i + 1) for i in range(0,5)))
    print('   '+'-'*len(" " + " ".join(map(str, range(1, 6)))))
    for i, fila in enumerate(mapa):
        fila_simbolos = [simbolo_celda(celda) for celda in fila]
    
        mapa_oculto.append((i, '|' , fila_simbolos))

    for numero_fila, fila, fila_simbolos in mapa_oculto:
        for j in range(5):
            if (numero_fila,j) == (posicion_jugador[0],posicion_jugador[1]):
                fila_simbolos[j] = 'J'
    for numero_fila, fila, fila_simbolos in mapa_oculto:

        print(f'{numero_fila+1} {fila} {" ".join(fila_simbolos)}')
    print('   '+'-'*len(" " + " ".join(map(str, range(1, 6)))))
...
def muestra_estado_mapa(mapa, posicion_jugador):
    ...
    posicion_jugador= (posicion_jugador[0]+1,posicion_jugador[1]+1)
